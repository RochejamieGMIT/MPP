...
class Customer:
    ...
    def CbuyingCSV(self,s):
        ...
        flag = 0                
        self.order_cost()
        print(self) # check products in shopping list
            ...
            for Sproduct in s.stock:
                if Sproduct.product.name == Cproduct.product.name:
                    ...
                    if Cproduct.quantity > Sproduct.quantity:
                        ...
                        Sproduct.quantity = Sproduct.quantity - Cproduct.quantity # change stock value
        self.calculate_costs(s.stock)
        orderCost = round(self.order_cost(),2) # calucualte order cost 
        if orderCost > self.budget:
            flag = 2
        if flag == 0:
            print(f'Total cost to {self.name} = {orderCost}') # update stock file with new stock values and cash
...
class Shop:
    # read shop from csv, this also takes in temp live buying csv
    def __init__(self, path):
        self.stock = []
        with open(path) as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=',')
    ...
    def __repr__(self):
        ...
        for item in self.stock:
            str += f"{item}\n"
        return str
    # get cash value in shop
    def getCash(self):
        str = ""
        str += f'Shop has {self.cash} in cash\n'
        return str
    #Live Buying create a temp csv to store live buying from user
    def SLiveBuy(self):
        print("Menu\n=========================")
        print(self)
        print("\n=========================")
        ...
        budget = input("Enter your budget: ") # take budget from user
        ...
        while True:
            ...
            if (option == "Coke Can"):
                ...
                #print(stockout["products"]) # remove duplicates in live buying
    ...
    def existingCustS(self):
        ...
        while True:
            ...
            option = input("Select: ") # take menu input from user 
            if (option == "1"):
                # https://builtin.com/data-science/python-list-files-in-directory
                # print the file names stored in customers folder
                exist = os.listdir("..\\Customers")
                print("Existing Customers \n------------------- \n")
                for x in exist:
                    print(x[:-4]) # remove .csv to print only names
                option2 = input("\nWhat is your name from the list above?")
                csvOut = option2.lower()+ ".csv" # add .csv to customer input to check if files match
                
                if csvOut in exist:
                    CustDir = ("..\\Customers\\" + csvOut) # pass the csv file to buy
                    c = Customer(CustDir) 
                    c.CbuyingCSV(self)
                    break

                else: 
                    print(f'Sorry {option2} is not a customer\nReturning to Main Menu\n')
                    break
...
def main():
    while True:
        s = Shop("..\\stock.csv") # read shop on loop to keep shop values current
